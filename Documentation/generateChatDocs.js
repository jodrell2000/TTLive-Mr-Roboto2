...
fs.writeFileSync( outputFile, "<html><body><style type=\"text/css\">\n" +
...
fs.readFile( dataFilePath,
    ...
    function ( err, data ) {
        ...
        theCommands.forEach( ( key, index ) => {
            ...
            fs.writeFileSync( outputFile, "<tr>\n" + "<td class=\"tg-0lax\">" + key + "</td>", { flag: 'a+' } );

            // write the messages in an unordered list
            fs.writeFileSync( outputFile, "<td class=\"tg-0lax\"><ul>", { flag: 'a+' } );
            theMessages.forEach( ( key, index ) => {
                // console.log( key );
                fs.writeFileSync( outputFile, "<li>" + key, { flag: 'a+' } );
            } );
            fsteFileSync( outputFile, "</ul></td>", { flag: 'a+' } );

            // write the pictures as 100 wide images in an unordered list
            // console.log( thePictures );
            fs.writeFileSync( outputFile, "<td class=\"tg-0lax\">", { flag: 'a+' } );
            if ( thePictures !== undefined ) {
                thePictures.forEach( ( key, index ) => {
                    // console.log( key );
                    fs.writeFileSync( outputFile, "<img src=\"" + key + "\" width=\"100\"><br>", { flag: 'a+' } );
                } );
            }
            fs.writeFileSync( outputFile, "</td>", { flag: 'a+' } );

            fs.writeFileSync( outputFile, "</tr>", { flag: 'a+' } );
        } );

        fs.writeFileSync( outputFile, "</tbody>\n" +
            "</table>\n" +
